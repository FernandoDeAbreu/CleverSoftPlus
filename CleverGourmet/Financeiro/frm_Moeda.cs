...
{
    public partial class frm_Moeda : Modelo
    {
        ...
        public override void pesquisar_Registro()
        {
            ...
            SQLCunsultaEmpr = "SELECT * FROM TBMOEDA WHERE DTEXCLUSAO IS NULL AND MOEDA " + descricao;
            ...
            while (conexao.dataReader.Read())
            {
                ...
                dgv_resultado_pesquisa.Rows[o].Cells["MOEDA"].Value = conexao.dataReader[1].ToString();
        ...
        public override void gravar_Registro()
        {
            ...
            try
            {
                if (tboxID.Text == "")
                {
                    #region INSERT
                    conexao.Abre_Conexao();
                    string SQLCunsultaEmpr = "INSERT INTO TBMOEDA " +
                                                      "(MOEDA     " +
                                                      ",STATUS)       " +
                                                      "VALUES         " +
                                                      "(@MOEDA    " +
                    ...
                    conexao.cmd.Connection = conexao.conexao;
                    conexao.cmd.CommandText = SQLCunsultaEmpr;
                    conexao.cmd.Parameters.AddWithValue("MOEDA", tboxcategoria.Text);
                ...
                else
                {
                    #region UPDATE
                    conexao.Abre_Conexao();
                    string SQLCunsultaEmpr = "UPDATE TBMOEDA SET " +
                                                      "MOEDA    = @MOEDA     " +
        ...
        public override void editar_Registro()
        {
            try
            {
                if (dgv_resultado_pesquisa.CurrentRow.Cells["MOEDA"].Value.ToString() == "DINHEIRO")
                {
                ...
                else
                {
                    ...
                    tboxcategoria.Text = dgv_resultado_pesquisa.CurrentRow.Cells["MOEDA"].Value.ToString();
        ...
        public override void excluir_Registro()
        {
            try
            {
                if (MessageBox.Show("Realmente deseje excluir o item selecionado?", "Clever Sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    ...
                    conexao.Abre_Conexao();
                    string SQLCunsultaEmpr = "UPDATE TBMOEDA SET DTEXCLUSAO = '" + DateTime.Now + "' WHERE ID = " + id_registro;
        ...
        public override void relatorio1()
        {
            try
            {
                ...
                a.Arquivo_rdlc = "Rpv_Moeda.rdlc";
                a.Sql_Relatorio1 = this.SQLCunsultaEmpr;
                a.Dataset_Relatorio1 = "DataSet_Moeda";
                a.ShowDialog();
            }
            catch (Exception)
            {
        ...
        private void frm_Moeda_Load(object sender, EventArgs e)
        {
            Iniciar_Form();
        }
    }
